...
sap.viz.ui5.data.transformers.ModelToFlattable.prototype._createVIZFlatTable=function(b,d,m){var c=b&&b.getContexts(0,b.getLength());var a=[],M=[],f={'metadata':{'fields':[],'context':'_context_row_number'},'data':[]},C=[];jQuery.each(d,function(i,o){a.push({def:o,adapter:o._getAdapter()});f.metadata.fields.push({'id':o.getName(),'name':o.getName(),'semanticType':'Dimension'});});jQuery.each(m,function(i,o){M.push({def:o,adapter:o._getAdapter()});f.metadata.fields.push({'id':o.getName(),'name':o.getName(),'semanticType':'Measure'});});if(!c||c.length===0){this.reset();this._oVIZFlatTable=new sap.viz.api.data.FlatTableDataset(f);return;}jQuery.each(c,function(I,o){if(!f.data[I]){f.data[I]=[];}for(var i=0;i<a.length;i++){var v=a[i].adapter(o);f.data[I].push(v);}for(var j=0;j<M.length;j++){var v=M[j].adapter(o);f.data[I].push(v);}C[I]=o;});this._aFlatContextLookup=C;this._oVIZFlatTable=new sap.viz.api.data.FlatTableDataset(f);};
